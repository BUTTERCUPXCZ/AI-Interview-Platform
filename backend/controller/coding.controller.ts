...
interface CodingQuestionRequest {
    domain: string
    difficulty: string
    language: string
    sessionId?: number
...
export const submitAnswer = async (req: Request, res: Response) => {
    try {
        ...
        const submissionData = {
            ...
            userId: (req as any).user?.id // Assuming you have user authentication
        }
        ...
        if (sessionId) {
            const savedQuestion = await prisma.interviewQuestion.create({
                data: {
                    ...
                    codingLanguage: language.toUpperCase() as any, // Cast to bypass enum check
            ...
            if (questionData.testCases && questionData.testCases.length > 0) {
                await Promise.all(
                    questionData.testCases.map((testCase: any) =>
