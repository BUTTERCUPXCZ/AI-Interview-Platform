    ...
    res.json = function (data) {
        ...
        try {
            ...
            if (data !== null && typeof data === "object" && "_cacheHit" in data) {
                ...
                payloadHit = data._cacheHit === true;
            }
 ...
 */
export const getEndpointAnalytics = async (endpoint, method, days = 7) => {
    const analytics = {
    ...
    try {
        for (let i = 0; i < days; i++) {
            ...
            const dayMetrics = (await CacheService.get(metricsKey)) || [];
        ...
        if (analytics.totalRequests > 0) {
            ...
            analytics.cacheHitRate = Math.round((analytics.dailyBreakdown.reduce((sum, day) => sum + (day.cacheHitRate * day.requests), 0) / analytics.totalRequests));
            analytics.errorRate = Math.round((analytics.dailyBreakdown.reduce((sum, day) => sum + (day.errorRate * day.requests), 0) / analytics.totalRequests));
        }
    }
    catch (error) {
