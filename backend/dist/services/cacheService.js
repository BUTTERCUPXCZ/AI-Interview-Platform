...
export class CacheService {
    ...
    static async setInterviewState(sessionId, state) {
        const key = `${CACHE_KEYS.INTERVIEW_STATE}${sessionId}`;
        return await redisOperations.set(key, state, CACHE_TTL.INTERVIEW_STATE);
    }
    static async getInterviewState(sessionId) {
        const key = `${CACHE_KEYS.INTERVIEW_STATE}${sessionId}`;
        return await redisOperations.get(key);
    }
    static async deleteInterviewState(sessionId) {
        const key = `${CACHE_KEYS.INTERVIEW_STATE}${sessionId}`;
        return await redisOperations.del(key);
    }
    // Coding session management
    static async setCodingSession(sessionId, sessionData) {
        const key = `${CACHE_KEYS.CODING_SESSION}${sessionId}`;
        return await redisOperations.set(key, sessionData, CACHE_TTL.CODING_SESSION);
    }
    static async getCodingSession(sessionId) {
        const key = `${CACHE_KEYS.CODING_SESSION}${sessionId}`;
        return await redisOperations.get(key);
    }
    static async deleteCodingSession(sessionId) {
        const key = `${CACHE_KEYS.CODING_SESSION}${sessionId}`;
        return await redisOperations.del(key);
    }
    // User progress caching
    static async setUserProgress(userId, progressData) {
    ...
    static async invalidateUserProgress(userId) {
        const key = `${CACHE_KEYS.USER_PROGRESS}${userId}`;
        return await redisOperations.del(key);
    }
    // Rate limiting
    static async setRateLimit(identifier, attempts = 1) {
    ...
    static async clearUserCaches(userId) {
        const promises = [
