...
export async function generateTextInterviewQuestionsOptimized(domain, difficulty, interviewType) {
...
export async function generateSingleTextQuestion(domain, difficulty, interviewType, questionNumber) {
    try {
        const model = genAI.getGenerativeModel({
            model: MODEL_NAME,
            generationConfig: {
...
export async function evaluateTextAnswerOptimized(question, answer) {
    try {
        const model = genAI.getGenerativeModel({
            model: MODEL_NAME,
            generationConfig: {
...
function getFallbackQuestions(domain, difficulty, interviewType) {
    const fallbackData = {
        technical: {
            frontend: [
    ...
    const categoryQuestions = fallbackData[interviewType];
    if (categoryQuestions) {
        const domainQuestions = categoryQuestions[domain] ||
...
export async function preWarmCache() {
    ...
    const promises = commonCombinations.map(([domain, difficulty, type]) => generateTextInterviewQuestionsOptimized(domain, difficulty, type)
